...
const ALERT_CONFIG = {
  "High Alert": {
    ...
    vibrate: [200, 100, 200, 100, 200], freq: [660, 660, 880],
  },
  "Very High Risk": {
    ...
    vibrate: [400, 150, 400, 150, 600], freq: [880, 880, 1100],
  }
}

// Persistent AudioContext — fixes "rings only once" bug
...
export default function VolunteerAlert() {
  ...
  function connect() {
    ...
    ws.onmessage = (e) => {
      try {
        ...
        if (!["High Alert", "Very High Risk"].includes(risk)) return
        ...
        if (Notification.permission === "granted") {
          new Notification(
            ...
            { body: `Crowd density: ${density}. ${ALERT_CONFIG[risk]?.msg}` }
          )
        }
      } catch (err) {
  ...
  async function changeUsername(e) {
    ...
    try {
      ...
      setProfileMsg({ type: "success", text: `Username changed to "${data.username}". Please log in again.` })
  ...
  const hasAlert     = activeAlerts.length > 0 && !acked
  const worstRisk = activeAlerts.some(([, c]) => c.risk === "Very High Risk") ? "Very High Risk" : "High Alert"
  const alertCfg     = ALERT_CONFIG[worstRisk]
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white font-mono">
      ...
      {/* ── Alert Overlay ─────────────────────────────────────────── */}
      {hasAlert && (
        <div className={`fixed inset-0 z-50 ${alertCfg.bg} flex flex-col items-center justify-center p-6 text-center ${worstRisk === "Very High Risk" ? "animate-pulse" : ""}`}>
          <div className="text-8xl mb-4">{alertCfg.emoji}</div>
          <h1 className={`text-4xl font-black tracking-wider mb-2 ${alertCfg.text}`}>{alertCfg.title}</h1>
              ...
              return (
                <div key={camId} className={`border ${cfg.border} rounded-2xl px-5 py-3 bg-black/30`}>
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${cfg.text} border ${cfg.border}`}>{cam.risk}</span>
                  </div>
                  <p className="text-3xl font-black mt-1" style={{ color: cam.risk === "Very High Risk" ? "#fca5a5" : "#fed7aa" }}>
                    {cam.density} <span className="text-sm font-normal text-gray-400">people</span>
                  </p>
                </div>
              )
            })}
          ...
          <button onClick={() => { setAcked(true); setAlertCams({}) }}
          ...
          <button onClick={() => playAlertSound(worstRisk)} className="mt-3 text-xs text-gray-400 underline">
            🔔 Replay Sound
          </button>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4" onClick={() => { setShowProfile(false); setProfileMsg(null) }}>
          <div className="bg-gray-900 border border-gray-700 rounded-2xl p-6 w-full max-w-sm" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => { setShowProfile(false); setProfileMsg(null) }} className="text-gray-500 hover:text-white">✕</button>
            </div>
            ...
            {profileTab === "password" && (
              <form onSubmit={changePassword} className="space-y-3">
                ...
                <button type="submit" className="w-full bg-emerald-700 hover:bg-emerald-600 text-white font-bold text-sm py-2.5 rounded-xl transition-all">
                  Update Password
                </button>
              </form>
            )}
            ...
            {profileTab === "username" && (
              <form onSubmit={changeUsername} className="space-y-3">
                ...
                <div>
                  <label className="text-xs text-gray-500 uppercase tracking-widest block mb-1">Confirm Password</label>
                ...
                <button type="submit" className="w-full bg-blue-700 hover:bg-blue-600 text-white font-bold text-sm py-2.5 rounded-xl transition-all">
                  Update Username
                </button>
              </form>
            )}
      ...
      <div className="max-w-sm mx-auto p-5">
        ...
        <div className="flex items-center justify-between py-4 mb-5">
          <div>
            ...
            <p className="text-xs text-gray-500">
              ...
              <span className="ml-1 text-gray-600">· Volunteer</span>
            </p>
          </div>
        ...
        <div className="bg-emerald-950/30 border border-emerald-900 rounded-3xl p-8 text-center mb-5">
          ...
          <p className="text-xs text-gray-500 mt-2 leading-relaxed">
            You'll be alerted on <span className="text-orange-400">High Alert</span> and{" "}
            <span className="text-red-400">Very High Risk</span>
          </p>
        </div>
          ...
          {history.length === 0 ? (
            ...
            return (
              <div key={i} className="flex items-center justify-between px-4 py-3 border-b border-gray-800 last:border-0">
                ...
                <p className="text-lg font-black text-white">{a.density}</p>
              </div>
            )
          })}
        </div>
